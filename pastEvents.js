 ...
 const mainArray = (data) => {
    ...
    function filtro() {
        ...
        if (textoVacio !== "" && arrayChecked.length > 0) {
            ...
            filterEvent.push(...arrayUpcoming.filter(buscar => buscar.name.toLowerCase().includes(textoVacio.toLowerCase().trim()) && arrayChecked.some(categoria => categoria == buscar.category)))
